...
def load_resources():
    ...
    ambience = pygame.mixer.Sound(os.path.join(SOUNDS_DIR, "ambience.mp3"))


    print('Resources loaded!')

    return goldfish, catfish, angelfish, bass, trout, anchovy, clownfish, crab, pufferfish, surgeonfish, worm, rusty_can, menu_bg, ocean_bg, moving_waves, start_button, finish_button, title_music, ambience

def load_resources_fish_handler():
    # B - Fresh Water
    goldfish = pygame.image.load(os.path.join(IMAGES_DIR, 'Fish/B - Fresh Water/Goldfish.png')).convert_alpha()
    catfish = pygame.image.load(os.path.join(IMAGES_DIR, 'Fish/B - Fresh Water/Catfish.png')).convert_alpha()
    angelfish = pygame.image.load(os.path.join(IMAGES_DIR, 'Fish/B - Fresh Water/Angelfish.png')).convert_alpha()
